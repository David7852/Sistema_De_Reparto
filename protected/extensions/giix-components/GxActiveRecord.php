...
abstract class GxActiveRecord extends CActiveRecord {
	...
	public function __toString() {
		$representingColumn = $this->representingColumn();
                $char="-";
                if(is_array($representingColumn)&&($representingColumn[0]==":"||$representingColumn[0]==", "||$representingColumn[0]==". "||$representingColumn[0]==" "||$representingColumn[0]==","||$representingColumn[0]==" : "||$representingColumn[0]==": "))
                {
                    $char=array_shift($representingColumn);
                }
		...
		if (is_array($representingColumn)) {
			$part = '';
                        if($char!=="-")
                        {
                            foreach ($representingColumn as $representingColumn_item) {
                                    ...
                                    $part .= ( $this->$representingColumn_item === null ? '' : $this->$representingColumn_item) . $char;
                            }
                        }else
                        {
                            foreach ($representingColumn as $representingColumn_item) {
			...
			return substr($part, 0, -1*strlen($char));
		} else {
	...
	public function findAllAttributes($attributes = null, $withPk = false, $condition='', $params=array()) {
		...
		if ($attributes === null)
			...
			$attributes = $this->representingColumn();
                        if(is_array($attributes)&&($attributes[0]==":"||$attributes[0]==", "||$attributes[0]==". "||$attributes[0]==" "||$attributes[0]==","||$attributes[0]==" : "||$attributes[0]==": "))
